...
const DONE_GROUPS: { g: string; items: string[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      "人間承認ゲートでルール昇格",
      "承認ルールを助言へ自動注入＋活用回数",
      "暗黙知の意味検索リトリーバ（Elastic／関連度で上位選択・安全フロア）",
      "社内ルールの手動追加・編集",
...
const EXTENSIONS = [
  { t: "暗黙知のフル意味検索（Elasticベクトル＋リランク）", d: "多言語埋め込み(e5)で、喋り言葉でも“意味”で暗黙知を引く。現在のElastic関連度検索(BM25)を、ハイブリッド＋セマンティック・リランクへ高度化。" },
...
const TACIT_ROADMAP = [
  ...
  { t: "対応の“思考状態”をパケット化して引き継ぐ", d: "各対応の根拠・確信度・新規性などを多次元で記録し、似た思考状態の過去対応をElasticで近傍検索。セッションをまたいで“判断の文脈”を継承する（思考座標ナビ）。" },
